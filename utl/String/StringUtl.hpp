...
namespace MXC::StringUtl {
    ...
    class [[deprecated("Use Split function in String::Utl")]] StringSplitter
        : public
          object{ public: StringSplitter(): object("MXC::StringUtl::StringSplitter", 1){}
...
static void inline Split(const char *const cstyle_str, const char *const delimiters,
                         StringContainer &des, size_t exit_length = 0) {
    gl_str buffer;
...
static void inline Split(const char *const cstyle_str, const char *const delimiters,
    ...
    for (auto *p = const_cast<char *>(delimiters); *p; ++p) { delimiters_set.insert(*p); }
...
static gl_str inline Join(const char *const str, IterableStringContainer &container) { }

static gl_str inline Replace(gl_str &string, gl_str &ptn, gl_str &replace_to,
                             bool greedy = true) {
    return "";
}

static inline void LeftPad(gl_str &str, const int32 size, char ch = ' ') {
    if (size > str.size()) str.insert(0, size - str.size(), ch);
}
}
